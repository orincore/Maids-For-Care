...
export default function EditMaidPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchServices(token), fetchProvider(token)]);
  }, [id, router]);
  ...
  const fetchProvider = async (token: string) => {
    try {
      ...
      setBio(p.bio || '');
      setSpecializationsRaw((p.specializations || []).join(', '));
      setLanguagesRaw((p.languages || []).join(', '));
      setSelectedServices((p.services || []).map((s: { _id: string } | string) => typeof s === 'string' ? s : s._id));
      ...
      setDocs({
        aadharCard: p.documents?.aadharCard || '',
        panCard: p.documents?.panCard || '',
        experienceCertificate: p.documents?.experienceCertificate || '',
      });
    } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/service-providers/${id}`, {
        method: 'PATCH',
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="sticky top-0 z-30 bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            <button onClick={() => router.back()} className="p-2 rounded-lg hover:bg-gray-100 text-gray-500 transition-colors flex-shrink-0">
              <ArrowLeft className="w-5 h-5" />
            </button>
            <div className="min-w-0">
              <h1 className="text-lg font-bold text-gray-900 leading-tight truncate">Edit Maid Profile</h1>
              <p className="text-xs text-gray-500 hidden sm:block truncate">{name}</p>
            </div>
          </div>
          <button
            type="submit"
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        <form id="maid-edit-form" onSubmit={handleSubmit} className="space-y-8">

          {/* ── Profile Photo ── */}
          <section className="bg-white rounded-2xl shadow-sm ring-1 ring-gray-200 p-6">
            <h2 className={SECTION}><ImageIcon className="w-4 h-4 text-gray-400" />Profile Photo</h2>
            <div className="flex items-center gap-5">
              ...
              <div className="flex-1">
                ...
                <button type="button" onClick={() => profileInputRef.current?.click()} disabled={profileUploading}
                  ...
                  {profileUploading ? <div className="w-4 h-4 border-2 border-gray-400 border-t-gray-700 rounded-full animate-spin" /> : <Upload className="w-4 h-4" />}
                  {profileUploading ? 'Uploading…' : profileImage ? 'Change Photo' : 'Upload Photo'}
                </button>
                {profileImage && (
                  <button type="button" onClick={() => setProfileImage('')} className="ml-2 text-sm text-red-500 hover:text-red-700">Remove</button>
                )}
          ...
          <section className="bg-white rounded-2xl shadow-sm ring-1 ring-gray-200 p-6">
            <h2 className={SECTION}><User className="w-4 h-4 text-gray-400" />Personal Information</h2>
          ...
          {/* ── Address ── */}
          <section className="bg-white rounded-2xl shadow-sm ring-1 ring-gray-200 p-6">
            <h2 className={SECTION}><MapPin className="w-4 h-4 text-gray-400" />Address</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="sm:col-span-2">
              ...
              <div>
                ...
                <input type="text" value={zipCode} onChange={e => setZipCode(e.target.value)} className={INPUT} />
              </div>
            </div>
          </section>
          ...
          <section className="bg-white rounded-2xl shadow-sm ring-1 ring-gray-200 p-6">
            <h2 className={SECTION}><Briefcase className="w-4 h-4 text-gray-400" />Professional Details</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div>
              ...
              <div>
                ...
                <input type="number" min={0} value={discountedPrice} onChange={e => setDiscountedPrice(e.target.value)} className={INPUT} />
              </div>
              <div className="sm:col-span-2">
          ...
          {services.length > 0 && (
            <section className="bg-white rounded-2xl shadow-sm ring-1 ring-gray-200 p-6">
              <h2 className={SECTION}>
                ...
                Assign Services
                <span className="ml-auto text-xs font-normal text-gray-400 normal-case tracking-normal">{selectedServices.length} selected</span>
              </h2>
                ...
                {services.map(svc => {
                  ...
                  return (
                    <button key={svc._id} type="button" onClick={() => toggleService(svc._id)}
                      className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-sm font-medium transition-all text-left ${on ? 'bg-gray-900 text-white border-gray-900 shadow-sm' : 'bg-white text-gray-700 border-gray-200 hover:border-gray-400'}`}>
          ...
          {/* ── Documents ── */}
          <section className="bg-white rounded-2xl shadow-sm ring-1 ring-gray-200 p-6">
            <h2 className={SECTION}><FileText className="w-4 h-4 text-gray-400" />Documents</h2>
          ...
          {/* ── Status ── */}
          <section className="bg-white rounded-2xl shadow-sm ring-1 ring-gray-200 p-6">
            <h2 className={SECTION}><ShieldCheck className="w-4 h-4 text-gray-400" />Status &amp; Verification</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Toggle label="Mark as Verified" description="Documents checked and approved" enabled={isVerified} color="emerald" onChange={() => setIsVerified(p => !p)} />
              <Toggle label="Set as Active" description="Visible to customers for booking" enabled={isActive} color="blue" onChange={() => setIsActive(p => !p)} />
            </div>
          </section>

          <div className="flex justify-end gap-3 pb-4">
            <button type="button" onClick={() => router.back()} className="px-6 py-2.5 border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50 transition-colors">
              Cancel
            </button>
            <button type="submit" disabled={saving} className="inline-flex items-center gap-2 bg-gray-900 text-white px-6 py-2.5 rounded-lg text-sm font-semibold hover:bg-gray-700 disabled:opacity-50 transition-colors">
  ...
  return (
    <div className="flex items-start gap-4 p-4 rounded-xl border border-gray-200 bg-gray-50/50">
      <div className="w-16 h-16 rounded-xl bg-white border border-gray-200 flex items-center justify-center flex-shrink-0 overflow-hidden">
        {url ? (
          isImage ? <img src={url} alt={label} className="w-full h-full object-cover" /> : <FileText className="w-7 h-7 text-blue-500" />
        ) : (
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button type="button" onClick={() => ref.current?.click()} disabled={uploading}
            ...
            {uploading ? <div className="w-3.5 h-3.5 border-2 border-gray-400 border-t-gray-700 rounded-full animate-spin" /> : <Upload className="w-3.5 h-3.5" />}
...
function Toggle({ label, description, enabled, color, onChange }: {
  ...
  const track = enabled ? color === 'emerald' ? 'bg-emerald-500' : 'bg-blue-500' : 'bg-gray-200';
  return (
    <div className="flex items-center justify-between gap-4 bg-gray-50 rounded-xl px-4 py-3.5 border border-gray-200">
      ...
      <button type="button" onClick={onChange}
        className={`relative inline-flex h-6 w-11 flex-shrink-0 rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${track}`}>
